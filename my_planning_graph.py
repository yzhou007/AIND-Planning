...
class PgNode_s(PgNode):
    '''
    A planning graph node representing a state_map (literal fluent) from a planning
    problem.
...
class PlanningGraph():
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for a1_effect_add in node_a1.action.effect_add:
            for a2_effect_rem in node_a2.action.effect_rem:
                if a1_effect_add.__eq__(a2_effect_rem):
                    return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for a1_effect_add in node_a1.action.effect_add:
            for a2_precon_neg in node_a2.action.precond_neg:
                if a1_effect_add.__eq__(a2_precon_neg):
                    return True
        
        for a1_effect_rem in node_a1.action.effect_rem:
            for a2_precon_pos in node_a2.action.precond_pos:
                if a1_effect_rem.__eq__(a2_precon_pos):
                    return True
        
        for a2_effect_add in node_a2.action.effect_add:
            for a1_precon_neg in node_a1.action.precond_neg:
                if a2_effect_add.__eq__(a1_precon_neg):
                    return True
        
        for a2_effect_rem in node_a2.action.effect_rem:
            for a1_precon_pos in node_a1.action.precond_pos:
                if a2_effect_rem.__eq__(a1_precon_pos):
                    return True
